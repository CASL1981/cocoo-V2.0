<div class="row">
    <div class="col-12 grid-margin">
      <x-otros.view-card :exportable="$exportable" :audit="$audit">
        <x-slot name="title">Empleados <small>employee</small></x-slot>
        <x-slot name="button">
        ...
        <x-table.table :audit="$audit">
          ...
          @forelse ($employees as $key => $item)
            <tr>
              ...
              <x-table.td class="text-right">{{ $item->identification }}</x-table.td>
              ...
              <x-table.td class="text-right">{{ $item->phone }}</x-table.td>
              <x-table.td class="text-right">{{ $item->cel_phone }}</x-table.td>
              <x-table.td class="text-center">{{ $item->entry_date }}</x-table.td>
              <x-table.td>{{ $item->email }}</x-table.td>
              <x-table.td  class="text-center">{{ $item->vendedor ? 'Si' : 'No' }}</x-table.td>
              <x-table.td class="text-center">{{ $item->gender }}</x-table.td>
              <x-table.td class="text-right">{{ $item->birth_date }}</x-table.td>
              <x-table.td class="text-right">{{ $item->location_id }}</x-table.td>
              <x-table.td class="text-center">{{ $item->approve ? 'Si' : 'No'}}</x-table.td>
            </tr>
          @empty
          <tr>
